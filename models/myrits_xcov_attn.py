...
#         print(self.rnn_hid_size)
        ...
        self.W = nn.Linear(self.rnn_hid_size, self.rnn_hid_size)
    ...
    def forward(self, data, direct):
        ...
        h_attn = Variable(torch.zeros((values.size()[0], 1, self.rnn_hid_size)))

        if torch.cuda.is_available():
            h, c, h_attn = h.cuda(), c.cuda(), h_attn.cuda()
        ...
        for t in range(self.seq_len):
            ...
            h, c = self.rnn_cell(inputs, (h, c)) # h = (batch_size, 50)
            if t==0:
                h_attn = h.reshape(h.shape[0], 1, -1)
            else:
                # score = self.W(h_attn) # score = W * hat_h_s
                # score = torch.bmm(score, h.unsqueeze(2)).squeeze() # score = score * h_t
                # attn_weights = F.softmax(score)
                h_attn = torch.cat([h_attn, h.unsqueeze(1)],dim=1)
        ...
        context_vector = torch.bmm(h_attn.permute(0,2,1), attn_weights.unsqueeze(2)).squeeze()
        ...
        hidden_size = context_vector.shape[1]
        h_hat = torch.mean(context_vector, dim=0)
        h_hat = h_hat.repeat(batch_size).reshape(batch_size,-1)
        h_dif = context_vector - h_hat
        ...
        return {'loss': x_loss * self.impute_weight + y_loss * self.label_weight, 'predictions': y_h,\
                'imputations': imputations, 'labels': labels, 'h': context_vector, 'weight': self.out.weight,\
