# coding: utf-8
# Computes the imputation performance of baseline models:
#   Mean
#   KNN
#   MICE
#   Matrix aActorization 
import json
import fancyimpute
import numpy as np
import pandas as pd
...
# Takes in formatted input data (with missing values)
datapath = "patdata24_new2/json"
modelpath = "7_23/elas/run1"
LOS = 24

for ctx in open(datapath):
...
X_c = np.concatenate(X, axis=0).reshape(-1, LOS, 65)
Y_c = np.concatenate(Y, axis=0).reshape(-1, LOS, 65)
# Z_c = np.array(Z)
X_mean = np.concatenate(X_mean, axis=0).reshape(-1, LOS, 65)
...
imp = np.load("{}/run_inference_impute.npy".format(modelpath))

print('RNN imputation')
print(get_loss(X_c, imp, Y_c))